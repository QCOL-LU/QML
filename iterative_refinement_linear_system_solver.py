...
def iterative_refinement_linear_system_solver(cofficent_matrix, rhs_vector, parameters):
    ...
    while (np.linalg.norm(r) > IRprecision):
        ...
        if parameters.Is_Quantum == False:
            lsa_solution    = np.linalg.solve(cofficent_matrix, nabla*r) + (noise() if parameters.Is_Noisy else np.zeros(d))
        
        else:
            lsa_solution    = QLSA(cofficent_matrix, nabla*r, parameters)[0]
            exact_solution  = np.linalg.solve(cofficent_matrix, nabla*r)
            QLSA_Precision  = np.linalg.norm(lsa_solution - exact_solution) / np.linalg.norm(exact_solution)
            parameters.QLSA_First_Run = False

        
        solution        = solution + (1/nabla)*lsa_solution
