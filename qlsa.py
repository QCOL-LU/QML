...
def QLSA(cofficent_matrix, rhs_vector, parameters):
	...
	#-----------------------------------------------------------------------
	# Ax = b => A.T Ax = A.T b
	# cofficent_matrix 	= np.matmul(cofficent_matrix.T, cofficent_matrix)
	# rhs_vector 			= np.matmul(cofficent_matrix.T, rhs_vector)
	...
	#-----------------------------------------------------------------------
		...
		if parameters.Is_Simulator == True:
			...
			if parameters.QLSA_First_Run == True:
				IBMQ.save_account(parameters.Token, overwrite=True)
				IBMQ.load_account()
			provider 	= IBMQ.get_provider(hub=parameters.Hub, group=parameters.Group, project=parameters.Project) 
			backend 	= provider.get_backend(parameters.QC_Name)

		hhl 			= HHL(parameters.qlsa_precision,  None, quantum_instance=backend )
